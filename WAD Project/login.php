...
<head>
    ...
    <title>Wellassa Eats | Login</title>
...
<body style="background-image: url(assets/img/login-cover.jpg); background-repeat: no-repeat; background-size: cover;">
            ...
            <div class="container"><a class="navbar-brand d-flex align-items-center" href="#"><img class="navbar-logo" src="assets/img/logo.png" style="width: 5rem;padding: 0.5rem;"></a><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-5"><span class="visually-hidden">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
                <div class="collapse navbar-collapse" id="navcol-5">
                    <ul class="navbar-nav ms-auto">
                        ...
                        <li class="nav-item"><a class="nav-link" href="#">Cart<i class="fas fa-cart-arrow-down" style="padding: 0.3rem;color: var(--bs-warning);"></i></a></li>
                    </ul><a class="btn btn-primary ms-md-2" role="button" href="signup.php" style="background: var(--bs-warning);">Signup</a>
                </div>
            </div>
        </nav>
    </div>
    ...
    //show error message to user
        if (isset($_GET["message"])) {
            if ($_GET["message"] == 1) {
                echo "<div class='alert'><span class='closebtn' onclick='this.parentElement.style.display=`none`;'>&times;</span><strong>Registered successfully!</strong></div>";
            } elseif ($_GET["message"] == 2) {
                echo "<div class='error'><span class='closebtn' onclick='this.parentElement.style.display=`none`;'>&times;</span><strong>Invalid Request Method!</strong></div>";
            } elseif ($_GET["message"] == 3) {
                echo "<div class='error'><span class='closebtn' onclick='this.parentElement.style.display=`none`;'>&times;</span><strong>Please click the 'Sign Up' button to complete the Signup process!</strong></div>";
            } elseif ($_GET["message"] == 4) {
                echo "<div class='error'><span class='closebtn' onclick='this.parentElement.style.display=`none`;'>&times;</span><strong>Incorrect email or password!</strong></div>";
            }
        }
    ?>
    <!--form start-->
